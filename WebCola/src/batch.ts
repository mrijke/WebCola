...
export function gridify(
  ...
  let gridrouter = route(
    pgLayout.cola.nodes(),
    pgLayout.cola.groups(),
    margin,
    groupMargin
  );
  return gridrouter.routeEdges<any>(
    pgLayout.powerGraph.powerEdges,
    nudgeGap,
    e => e.source.routerNode.id,
    e => e.target.routerNode.id
  );
}
  ...
  groups.forEach(d => {
    d.routerNode = <any>{
      ...
      children: (typeof d.groups !== "undefined"
        ? d.groups.map(c => nodes.length + c.id)
        : []
      ).concat(
        typeof d.leaves !== "undefined" ? d.leaves.map(c => c.index) : []
      ),
    };
  });
