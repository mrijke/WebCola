      ...
      .constraints([
        {
          type: "alignment",
          axis: "y",
          offsets: [{ node: 0, offset: 0 }, { node: 1, offset: 0 }],
        },
      ])
      .on(cola.EventType.start, e => starts++)
      .on(cola.EventType.tick, e => ticks++)
...
QUnit.test("single link", assert => {
  ...
  nodes.forEach(({ x, y }) =>
    assert.ok(Math.abs(x) < 1e-5 && Math.abs(y) < 1e-5)
  );
  assert.ok(
    Math.abs(linkLength - desiredLength) < 1e-4,
    "length = " + linkLength
  );
  ...
  assert.ok(
    Math.abs(linkLength - smallerLength) < 1e-4,
    "length = " + linkLength
  );
});
...
QUnit.test("Pyramid", assert => {
  ...
  lengths.forEach(l =>
    assert.ok(Math.abs(l - lengths[0]) < 1e-4, "length = " + l)
  );
});
...
QUnit.test("Fixed nodes", assert => {
  ...
  let check = () => {
      ...
      if (nodes[i].fixed)
        cola.Layout3D.dims.forEach((d, j) =>
          assert.ok(
            ...
            `nodes[${i}] lock in ${d}-axis at ${nodes[i][d]}, actual=${
              layout.result[j][i]
            }`
          )
        );
    ...
    lengths.forEach(l =>
      assert.ok(
        closeEnough(l, meanLength, meanLength / 20),
        "edge length = " + l
      )
    );
  };
